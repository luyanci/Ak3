...
patch_cmdline() {
  ...
  if ! grep -q "$1" $cmdfile; then
    ...
    $gnused -i -e 's;  *; ;g' -e 's;[ \t]*$;;' $cmdfile || echo "$gnused fails";
  else
    match=$(grep -o "$1.*$" $cmdfile | cut -d\  -f1);
    $gnused -i -e "s;${match};${2};" -e 's;  *; ;g' -e 's;[ \t]*$;;' $cmdfile || echo "$gnused fails";
  fi;
  if [ -f "$home/cmdtmp" ]; then
    $gnused -i "s|^cmdline=.*|cmdline=$(cat $cmdfile)|" $split_img/header || echo "$gnused fails";
    rm -f $cmdfile;
  fi;
}
